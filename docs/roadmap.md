...
- 配置 `ANTHROPIC_API_KEY` 后 `hnsx run --domain domains/financial-analysis/domain.yaml --trigger '{"ticker":"AAPL"}'` 跑通
- `hnsx run --domain domains/customer-service/domain.yaml`（用 noop）走完两个 step
...
## Phase 2 — Sandbox + Claude Code CLI

**Goal**: Claude Code 在 Linux namespace 沙箱里跑 PR review，文件操作被隔离。

| # | Deliverable | 位置 |
|---|---|---|
| 2.1 | Linux namespace 沙箱（mount + pid + net） | `hnsx-sandbox/src/linux/namespace.rs` |
| 2.2 | Landlock 文件访问规则 | `hnsx-sandbox/src/linux/landlock.rs` |
| 2.3 | Seccomp syscall 白名单 | `hnsx-sandbox/src/linux/seccomp.rs` |
| 2.4 | cgroups v2 资源配额（CPU / memory） | `hnsx-sandbox/src/linux/cgroup.rs` |
| 2.5 | `SandboxSpec` 补全字段：filesystem.mounts / commands.whitelist / network.domains / resources.quota / timeout.{total,idle} | `hnsx-core/sandbox.rs` |
| 2.6 | Claude Code CLI adapter（沙箱内 spawn + 流式 stdout） | `hnsx-adapter/claude_code.rs` |
| 2.7 | PII 行级检测（`Chunk::error`） | `hnsx-core/telemetry.rs` 钩子 |
| 2.8 | 沙箱 RAII 销毁（drop 时清理 namespace/cgroup） | `hnsx-sandbox/src/linux/mod.rs` |
| 2.9 | Linux CI job（`ubuntu-latest`）+ macOS 友好错误 | `.github/workflows/ci.yml` |

**Acceptance**
- Linux 上 `hnsx run --domain domains/code-review/domain.yaml` 走完一个真实 PR review
- 沙箱内写入不污染 host
- `cargo test -p hnsx-sandbox` 在 Linux CI 绿
- macOS dev 上跑 sandbox 报清晰错误而非 panic

**依赖**: Phase 1
**风险**: Landlock 需要 kernel ≥ 5.13（探测 + 降级）；seccomp 调试成本高（先只 deny 危险 syscall）
...
| # | Deliverable |
|---|---|
| 3.1 | `Tool` trait + `ToolSpec`（`hnsx-core`） |
| 3.2 | `hnsx-tool/http.rs` — GET/POST + Bearer/OAuth + 超时 |
| 3.3 | `hnsx-tool/shell.rs` — 跑在 sandbox 内，whitelist |
| 3.4 | `hnsx-tool/sql.rs` — Postgres（`sqlx`）+ SQLite（`rusqlite`） |
| 3.5 | `hnsx-tool/python.rs` — sandbox 内 Python 脚本 |
| 3.6 | 工具注册表 + AgentSpec 解析 |
| 3.7 | 重构 `financial-analysis` 让 `quoter` 用 `http` 工具 |
...
- **沙箱处理**：macOS 上 `hnsx-sandbox` 是空 stub，Linux 上才编译真实实现。`#[cfg(target_os = "linux")]` 隔离。
