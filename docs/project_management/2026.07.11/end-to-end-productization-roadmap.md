...
- [x] **server** `RunEval` 改为将每个 case 作为 session 请求 enqueue 到 worker pool
- [x] **server** 聚合 worker 回传的 observation 与 result，生成 EvalRun report
- [x] **server** scheduler 识别 `supervisor` / `hierarchical` / `autonomous` 模式并分发给 worker
- [x] **worker** eval runner 与 server 上报格式对齐（复用 `hnsx-worker/hnsx_worker/eval/runner.py`）
- [x] **examples** 确保 `claude-triage`、`workflow-demo` 走 server → worker 全链路跑通
