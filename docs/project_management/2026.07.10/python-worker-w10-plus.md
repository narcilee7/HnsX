...
| W12 | Phase 3 | 高级 Agent 编排 | Planning / Reflection / ReAct / Multi-Agent 协作 | Phase 2 memory、LLM 调用层 |
| W13 | Phase 4 | 智能 Eval 与 Harness 自进化 | Eval-driven improvement loop、regression radar、auto-prompt-evolution | Eval 平台、Trace DB |
| W14 | Phase 5 | 人机协同与审批增强 | Human-in-the-loop 任务挂起、协作编辑、异步审批 UI 协议 | server 审批 API、console |
...
### 5.3 验收标准
- 新增 `tests/agents/test_react_agent.py`、`tests/agents/test_multi_agent.py`。
- `example-domains/financial-analysis` 切换到 `plan_and_solve` 后效果提升。
- 多 Agent 协作 example 在 `example-domains/code-review` 中落地。
...
### 6.3 验收标准
- 新增 `tests/eval/test_improvement_loop.py`。
- 在 `example-domains/claude-triage` 跑一轮自动 prompt evolution，有明确 report。
- CI 中 regression radar 能识别人为注入的退化用例。
...
### 7.3 验收标准
- 新增 `tests/approval/test_human_in_the_loop.py`（模拟审批回调）。
- `example-domains/customer-service` 演示「发送退款前需要人工确认」。
- 审批中断后可恢复，Session 状态正确。
