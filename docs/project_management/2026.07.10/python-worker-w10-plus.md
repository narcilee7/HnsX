...
| W15 | Phase 6 | Python SDK 与插件化（核心 capabilities） | `skills/{registry,resolver}.py` + SkillRegistry + resolve_agent_skills；harness loader 交叉校验 `agent.skill_refs`；session_executor 与 harness/runner 两条执行路径都把 skill 的 tools / prompts / examples 注入到运行时 | SDK 子包拆分 / `hnsx_skill.toml` / 外部 skills/ / CLI / ACP |
...
### 8.3 验收标准
- [x] 新增 `hnsx_worker/skills/{__init__,registry,resolver}.py` —— `Skill` dataclass、`SkillRegistry`、`resolve_agent_skills` + 类型化错误 `SkillResolutionError`。`tests/skills/` 28 个单测全绿。
- [x] `harness/loader.py` 增 `self.skills`（`SkillRegistry.from_dict`） + `_validate_skills()`（任何 mode 都跑，校验 `agent.skill_refs ⊆ skills`）；`tests/test_harness.py` +8 校验测试全过。
- [x] `session_executor._build_tool_registry` 接 `extra_tool_specs`；`session_executor._resolve_skill_tool_specs` 把 skill 的 tools 抽到 agent 的 ToolRegistry。两处调用点（多 turn / W12 strategy）都已接入。
- [x] `harness/runner.py._resolve_skill_context` 把 skill 的 prompts + examples 写入 system prompt 前缀，supervisor 路由决策时知晓能力面。
- [x] 端到端 `tests/test_skill_integration.py` —— 含显式 agent tool 与 skill 同名时优先级的去重测试。
- [x] 示例域 `example-domains/skills-demo/domain.yaml` —— agent 零 inline `tools:` 配置，**完全靠 `skill_refs` 提供工具**（hello-world `greet` + time-now `now`），验证整套 wire 通过。
- [ ] `hnsx-sdk` 独立包拆分 + `@skill` 装饰器 + `tests/sdk/test_skill_decorator.py` —— 留待下一迭代（拆 SDK 需要单独的发布策略定调）。
- [ ] 外部 skill 包规范（`hnsx_skill.toml` + `skills/` 插件目录 + `hnsx skill install` CLI + 分发模式） —— 留待下一迭代（先决定 PyPI 公网 vs 内部源再实施）。
- [ ] 至少 3 个官方 skill 示例（web-search / memory / slack-notification） —— 当前 demo 含 hello-world + time-now 2 个，覆盖到基本能力，slack/web-search 等待 SDK 拆分一起上。
- [ ] ACP —— 用户明确「下个迭代再说」，用编排替代；`hnsx_worker/acp/` 留空目录现状不动。
