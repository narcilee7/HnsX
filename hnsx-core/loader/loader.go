// Package loader parses DomainSpec v2 YAML/JSON documents and validates them.
//
// v2 YAML shape (canonical):
//
//	id: customer-service
//	version: 0.1.0
//	description: ...
...
func LoadFile(path string) (*domain.DomainSpec, error) {
...
func Parse(data []byte) (*domain.DomainSpec, error) {
	spec := new(domain.DomainSpec)
...
func MustParse(data []byte) *domain.DomainSpec {
	spec, err := Parse(data)
	if err != nil {
		panic(err)
	}
