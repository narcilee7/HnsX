...
func TestParse_OK(t *testing.T) {
	spec, err := loader.Parse([]byte(baseSpec))
	if err != nil {
...
func TestValidate_NilSpec(t *testing.T) {
	if err := loader.Validate(nil); err == nil {
...
func TestValidate_FieldChecks(t *testing.T) {
	cases := []struct {
		...
		{
			name: "missing id",
			spec: &domain.DomainSpec{Version: "0.1.0", Harness: domain.HarnessSpec{
				Agents:  map[string]domain.AgentSpec{"a": {Provider: "anthropic", Model: "m"}},
				Session: domain.SessionSpec{Mode: "single"},
			}},
			want: "domain.id is required",
		},
		{
			name: "missing version",
			spec: &domain.DomainSpec{ID: "x", Harness: domain.HarnessSpec{
				Agents:  map[string]domain.AgentSpec{"a": {Provider: "anthropic", Model: "m"}},
				Session: domain.SessionSpec{Mode: "single"},
			}},
...
func TestMustParse_PanicOnError(t *testing.T) {
	...
	loader.MustParse([]byte("not yaml: : :"))
}
