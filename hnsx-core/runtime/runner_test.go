...
func TestRunner_SingleMode_HappyPath(t *testing.T) {
	spec := &domain.DomainSpec{
		...
		Harness: domain.HarnessSpec{
			...
			Session: domain.SessionSpec{Mode: "single", Agent: "main"},
		},
	}
	...
	res, err := runner.Run(context.Background(), spec, map[string]any{"question": "hi"})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRunner_WorkflowMode_WalksSteps(t *testing.T) {
	...
	res, err := runner.Run(context.Background(), spec, map[string]any{"trigger": "1"})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
	if res.State != "completed" {
...
func TestRunner_UnknownMode_ReturnsError(t *testing.T) {
	...
	_, err := NewRunner(&stubAdapter{}).Run(context.Background(), spec, nil)
...
func TestRunner_SupervisorMode_NotImplemented(t *testing.T) {
	spec := &domain.DomainSpec{
		...
		Harness: domain.HarnessSpec{
			Agents:  map[string]domain.AgentSpec{"a": {ID: "a", Provider: "p", Model: "m"}},
			Session: domain.SessionSpec{Mode: "supervisor"},
		},
	}
	_, err := NewRunner(&stubAdapter{}).Run(context.Background(), spec, nil)
