...
Runs the OpenAI ``codex`` CLI as a subprocess, streams its stdout as text
deltas, and heuristically detects shell/file/edit operations so the Harness
can audit and optionally deny them via Policy.
...
class CodexAdapter(CliAdapter):
    ...
    def _tool_patterns(self) -> list[CliToolPattern]:
        """Detect operations the Codex CLI commonly reports."""
        return [
            CliToolPattern(
                "bash",
                re.compile(
