...
// Broadcaster is a per-session pub/sub for observation.Observation values.
//
...
type Broadcaster struct {
	mu        sync.Mutex
	subs      map[chan observation.Observation]struct{}
	closed    bool
...
func NewBroadcaster() *Broadcaster {
	return &Broadcaster{subs: map[chan observation.Observation]struct{}{}, bufferCap: 256}
}
...
func (b *Broadcaster) Subscribe() (<-chan observation.Observation, func()) {
	ch := make(chan observation.Observation, 256)
	b.mu.Lock()
	if b.closed {
		b.mu.Unlock()
		close(ch)
	...
	replay := make([]observation.Observation, len(b.buffer))
...
func (b *Broadcaster) Publish(ctx context.Context, obs observation.Observation) error {
	b.mu.Lock()
	if b.closed {
		b.mu.Unlock()
		return nil
	...
	subs := make([]chan observation.Observation, 0, len(b.subs))
	for ch := range b.subs {
		subs = append(subs, ch)
	}
	b.mu.Unlock()
